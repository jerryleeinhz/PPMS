...
栅压扫描由`simulate-gate`和`run-hardware-gate`进入。`mode = "grid"`时，上下栅按独立
点数形成二维蛇形图；从图或独立电容标定确定零电场轨迹后，可改为`mode = "paired"`，用
相同点数和两对端点让Vg1/Vg2沿该直线同步变化。每个条件继续采集上述六路信号。代码不会
仅凭电阻自动宣称某条轨迹为零电场线。软件和仿真测试已完成，但真实非零栅压尚未在实验
设备/样品上验证，必须按`HARDWARE_VALIDATION_CHECKLIST.md`逐级确认。

`instrument_samples`逐样本保存两台Keithley的栅压、输出状态、compliance和漏电流；
样品电阻不是Keithley的`V/I`，而是输运导出中的锁相电压除以SR估算驱动电流。配置要求
软件漏电终止值严格低于Keithley compliance。
...
已从高校仪器中心公开页面下载并验证`docs/manuals/quantum_design/`中的ETO 1084-700 Rev. B2
和PPMS MultiVu 1070-110 Rev. A2。ETO手册覆盖软件、数据列、测量与ETO sequence命令；MultiVu
